...
insert into @MonthlySchedule(ID, Value) values(1, 'Sunday'),(2, 'Monday'),(3, 'Tuesday'),(4, 'Wednesday'),(5, 'Thursday'),(6, 'Friday'),(7, 'Saturday'),
(8, 'day'),(9, 'weekday'),(10, 'weekend day');

select
	MainPlans.name as "Plan Name"
	, SubPlans.subplan_description as "Sub-Plan Description"
	--, SubPlans.subplan_name as "Sub-Plan Name"
	--, Jobs.name as "Job Name"
	/*, case Packages.packagetype
	...
	, cast(cast(Packages.packagedata as varbinary(max)) as xml)  as "Package XML"
from dbo.sysmaintplan_plans as MainPlans
inner join dbo.sysmaintplan_subplans as SubPlans on MainPlans.id = SubPlans.plan_id
inner join dbo.sysssispackages as Packages on MainPlans.id = Packages.id
inner join dbo.sysjobs as Jobs on SubPlans.job_id = Jobs.job_id
left outer join dbo.sysjobschedules as JobSchedules on Jobs.job_id = JobSchedules.job_id
left outer join dbo.sysschedules as Schedules
on JobSchedules.schedule_id = Schedules.schedule_id
order by MainPlans.name
go
